...
namespace Leopotam.Ecs
{
    ...
    public class EcsWorld
    {
        ...
        public EcsFilter GetFilter(Type filterType, bool createIfNotExists = true)
        {
            ...
            // create new filter.
            var filter = (EcsFilter) Activator.CreateInstance(filterType, BindingFlags.NonPublic | BindingFlags.Instance, null,
                _filterCtor, CultureInfo.InvariantCulture);
#if DEBUG
            ...
            if (typeIdx < 0)
            {
                ...
                if (FilterByIncludedComponents.TryGetValue(-typeIdx, out filters))
                {
                    for (int i = 0, iMax = filters.Count; i < iMax; i++)
                    {
                        ...
                        {
#if DEBUG
                            if (!filters.Items[i].GetInternalEntitiesMap()
