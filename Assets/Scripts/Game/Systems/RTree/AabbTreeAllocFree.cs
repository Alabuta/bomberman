...
{
    public class AabbTreeAllocFree : IRTree
    {
        ...
        public void Build(EcsFilter<TransformComponent, HasColliderTag> filter)
        {
            ...
            foreach (var index in filter)
            {
                ...
                var leafEntry = new RTreeLeafEntry(aabb, index);
                Insert(leafEntry);
            }
        }
        ...
        private RTreeNode? ChooseLeaf(ref RTreeNode node, int nodeLevelIndex, in RTreeLeafEntry entry)
        {
...
#if ENABLE_PROFILING
            using var _ = Profiling.RTreeNodesUpdate.Auto();
#endif
            ...
            var childNodeIndex = GetNodeIndexToInsert(childNodeLevelIndex, entriesStartIndex, entriesEndIndex, false, entry);
#if ENABLE_ASSERTS
